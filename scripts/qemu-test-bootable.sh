#!/bin/bash
# test bootable-minimal system by extracting kernel and rootfs from zub
set -eu
...
if [ ! -f "$ZUB" ]; then
    ...
    bash -c "cd $ROOT_DIR/src/zub && cargo build"
fi

TMPDIR=$(mktemp -d)
trap "rm -rf $TMPDIR" EXIT

echo "=== Extracting kernel from zub ==="
$ZUB --repo="$REPO" checkout "$KERNEL_REF" "$TMPDIR/kernel"
...
echo "=== Extracting rootfs from zub ==="
$ZUB --repo="$REPO" checkout "$SYSTEM_REF" "$TMPDIR/rootfs"
