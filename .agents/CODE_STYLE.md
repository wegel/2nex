...
## 0. Keep the whole system small

Treat total production LoC as a design constraint. Keep the smallest design
that remains correct, robust, efficient, and easy to change.

Apply late-1990s suckless discipline to Rust: prefer small, complete mechanisms
with predictable costs. Use strong types when they make invalid states
unrepresentable and remove runtime branches; avoid ceremonial wrappers that
only move complexity into type declarations.

When strong types require more lines, accept the smallest type that prevents a
concrete invalid state, then delete the checks and branches it replaces. A
wrapper that cannot remove anything does not earn its cost.

- Make every new type, layer, dependency, and branch earn its cost.
- Remove superseded code when adding a new path.
- Do not hide bloat by spreading it across more files or tiny modules.
- Do not compress code into cryptic expressions merely to lower the count.
- Prefer one complete mechanism over several overlapping special cases.
- Compare the net production LoC before accepting a feature or refactor.

Small code is easier to audit and leaves fewer places for bugs to hide. Line
count never excuses weaker checks, vague errors, unsafe shortcuts, or missing
tests.
