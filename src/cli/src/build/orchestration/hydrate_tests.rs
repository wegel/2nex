...
fn hydrated_dependency_names_use_package_slug_not_pkg_segment() {
    let name = hydrated_dependency_name("x86_64/pkg/libs/system/glibc/2.39/bundles/dev", &[])
...
fn hydrated_dependency_names_preserve_direct_names() {
    ...
    let name = hydrated_dependency_name(&direct.commit, std::slice::from_ref(&direct))
        .expect("test setup should succeed");
...
fn formatted_hydrated_dependencies_use_meaningful_names() {
    ...
    let dependency = Dependency {
        name: hydrated_dependency_name(&commit, &[]).expect("test setup should succeed"),
        commit,
        manifest_ref: None,
    };
