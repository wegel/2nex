...
impl Ext4Fs {
    ...
    pub fn read_file(&self, path: &str) -> Result<Vec<u8>, Ext4Error> {
        ...
        self.inner.read(path).map_err(Ext4Error::Parse)
    }
    ...
    pub fn read_dir(&self, path: &str) -> Result<Vec<DirEntry>, Ext4Error> {
        ...
        let dir = self.inner.read_dir(path).map_err(Ext4Error::Parse)?;
...
pub fn mount(partition: &RootPartition) -> Result<Ext4Fs, Ext4Error> {
    ...
    let inner = ext4_view::Ext4::load(Box::new(reader)).map_err(Ext4Error::Parse)?;
...
pub fn mount_from_reader(reader: Box<dyn ext4_view::Ext4Read>) -> Result<Ext4Fs, Ext4Error> {
    ...
    let inner = ext4_view::Ext4::load(reader).map_err(Ext4Error::Parse)?;
...
impl ext4_view::Ext4Read for UefiBlockReader {
    fn read(
        ...
        let end_block = self.start_lba + end_byte.div_ceil(block_size);
