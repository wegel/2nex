...
fn unseal_from_tpm() -> Result<TpmKeyMaterial, TpmError> {
    // open TCG2 protocol
    let tcg_handle = uefi::boot::get_handle_for_protocol::<Tcg>()
    ...
    let key_data = nv_read_with_session(&mut tcg, session_handle, NV_INDEX_LUKS_MASTER_KEY, MASTER_KEY_SIZE)?;
...
fn start_policy_session(tcg: &mut Tcg) -> Result<u32, TpmError> {
    ...
    let session_handle = u32::from_be_bytes([response[10], response[11], response[12], response[13]]);
...
fn nv_read_with_session(tcg: &mut Tcg, session_handle: u32, nv_index: u32, size: u16) -> Result<Vec<u8>, TpmError> {
    // TPM2_NV_Read command (with sessions)
    // structure: tag(2) + size(4) + cc(4) + authHandle(4) + nvIndex(4) +
    //            authArea + size(2) + offset(2)
    let mut cmd = Vec::with_capacity(64);
    ...
    let nv_handle = 0x01000000 | nv_index; // NV index handle format
    cmd.extend_from_slice(&nv_handle.to_be_bytes()); // authHandle
    cmd.extend_from_slice(&nv_handle.to_be_bytes()); // nvIndex

    // authorization area size (4 bytes)
    let auth_area_start = cmd.len();
    cmd.extend_from_slice(&0u32.to_be_bytes()); // placeholder

    // authorization area: use policy session
    // sessionHandle(4) + nonce(2+0) + sessionAttributes(1) + hmac(2+0)
    cmd.extend_from_slice(&session_handle.to_be_bytes());
    cmd.extend_from_slice(&0u16.to_be_bytes()); // nonce size = 0
    cmd.push(0x01); // sessionAttributes: continueSession
    cmd.extend_from_slice(&0u16.to_be_bytes()); // hmac size = 0
    ...
    // NV_Read parameters
    cmd.extend_from_slice(&size.to_be_bytes()); // size to read
    cmd.extend_from_slice(&0u16.to_be_bytes()); // offset = 0
    ...
    let rc = parse_response_code(&response)?;
    if rc != TPM2_RC_SUCCESS {
        // 0x14b = NV not found, 0x98e = policy failure
        return Err(TpmError::CommandFailed(rc));
    }

    // response format: tag(2) + size(4) + rc(4) + parameterSize(4) + data(2+n) + authArea
    if response.len() < 16 {
        return Err(TpmError::InvalidResponse);
    }
    ...
    let data = response[16..16 + data_size as usize].to_vec();
    Ok(data)
}
