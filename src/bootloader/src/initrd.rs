...
use uefi::{Error, Handle, Identify, Status};

use crate::linux_efi::{self, InitrdDevicePath, LoadFile2Protocol, EFI_LOAD_FILE2_PROTOCOL_GUID};
...
pub fn install_initrd_protocol(initramfs_data: Vec<u8>) -> Result<Handle, Error> {
    ...
    log::info!("initrd: installed LoadFile2 protocol ({} bytes)", unsafe {
        (*INITRD_DATA.load(Ordering::SeqCst)).len()
    });
