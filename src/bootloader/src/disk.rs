...
pub fn find_root_partition() -> Result<RootPartition, BootError> {
    ...
    for &handle in handles.iter() {
        let Some(root) = root_partition_from_handle(handle) else {
            continue;
        };
        if partition_matches_boot_device(handle, boot_prefix.as_deref()) {
...
fn root_partition_from_handle(handle: Handle) -> Option<RootPartition> {
    let partition_info = uefi::boot::open_protocol_exclusive::<PartitionInfo>(handle).ok()?;
    let gpt = partition_info.gpt_partition_entry()?;
    ...
    let block_io = uefi::boot::open_protocol_exclusive::<BlockIO>(handle).ok()?;
...
fn partition_matches_boot_device(handle: Handle, boot_prefix: Option<&[u8]>) -> bool {
    let Some(boot_prefix) = boot_prefix else {
        return false;
    };
    device_path_prefix(handle).as_deref() == Some(boot_prefix)
}
