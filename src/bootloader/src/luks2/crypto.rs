...
fn derive_key_argon2(passphrase: &[u8], kdf: &Kdf, output_len: usize) -> Result<Vec<u8>, CryptoError> {
...
fn derive_key_pbkdf2(passphrase: &[u8], kdf: &Kdf, output_len: usize) -> Result<Vec<u8>, CryptoError> {
...
impl AesXts {
    ...
    pub fn decrypt_sector(&self, sector_index: u64, data: &mut [u8]) {
        ...
        self.xts.decrypt_sector(sector, get_tweak_default(sector_index as u128));
    }
    ...
    pub fn decrypt_sectors(&self, start_sector: u64, data: &mut [u8]) {
        ...
        for i in 0..num_sectors {
            ...
            self.xts.decrypt_sector(sector, get_tweak_default((start_sector + i as u64) as u128));
        }
    }
...
pub fn af_merge(split_key: &[u8], key_size: usize, stripes: u32, hash: &str) -> Result<Vec<u8>, CryptoError> {
    if hash != "sha256" {
        return Err(CryptoError::UnsupportedHash(String::from(hash)));
    }
...
fn af_diffuse(data: &[u8]) -> Vec<u8> {
    ...
    let num_blocks = (data.len() + hash_size - 1) / hash_size;
