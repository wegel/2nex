...
def enable_safe_gate_bools(kconf, gate_bools, dangerous_bools, explicit) -> bool:
    changed = False
    for sym in sorted(gate_bools, key=lambda s: s.name or ""):
        name = sym.name
        if not name:
            continue
        if sym.choice is not None:
            # Avoid fighting Kconfig choices (can cause flip-flopping)
            continue
...
def main():
    ...
    while enable_safe_gate_bools(
        kconf, gate_bools, dangerous_bools, explicit
    ):
        pass

    unresolved = []
    for gate in sorted(gate_bools, key=lambda s: s.name or ""):
        name = gate.name
